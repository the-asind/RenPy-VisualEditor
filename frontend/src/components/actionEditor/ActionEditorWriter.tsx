...
import {
  ...
  insertActionEditorEmptyRowAfter,
  projectActionEditorWriterRows,
  serializeActionEditorWriterRows,
  speakerAccentForId,
...
const commandLabelKeys = new Map<ActionEditorCommandRow['kind'], string>([
  ['scene', 'actionEditor.writer.command.scene'],
  ['show', 'actionEditor.writer.command.show'],
  ['hide', 'actionEditor.writer.command.hide'],
  ['music', 'actionEditor.writer.command.music'],
  ['sound', 'actionEditor.writer.command.sound'],
  ['transition', 'actionEditor.writer.command.transition'],
]);
...
const ACTION_EDITOR_TEXT_COMMIT_DEBOUNCE_MS = 750;

const speakerLabel = (row: ActionEditorDialogueRow | ActionEditorNarrationRow, t: TFunction): string =>
  row.kind === 'narration' ? t('actionEditor.writer.narrator') : speakerNames.get(row.speaker) ?? row.speaker;

const commandLabel = (kind: ActionEditorCommandRow['kind'], fallback: string, t: TFunction): string => {
  const key = commandLabelKeys.get(kind);
...
export const ActionEditorWriter = memo(function ActionEditorWriter({
  ...
  const rows = useMemo(() => projectActionEditorWriterRows(content), [content]);
  ...
  const pendingRowsRef = useRef<ActionEditorRow[] | null>(null);
  const pendingContentTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const emptyRowFocusTargetIndexRef = useRef<number | null>(null);
  const [emptyRowFocusRequestId, setEmptyRowFocusRequestId] = useState(0);
  const emptyRowFocusTargetIndex = emptyRowFocusTargetIndexRef.current;
  ...
  const flushPendingContent = () => {
    ...
    onContentChange(serializeActionEditorWriterRows(pendingRows));
  };

  const scheduleRowsChange = (nextRows: ActionEditorRow[]) => {
    if (onContentSplice) {
      const previousContent = serializeActionEditorWriterRows(draftRowsRef.current);
      const nextContent = serializeActionEditorWriterRows(nextRows);
      const splice = deriveActionEditorTextSplice(previousContent, nextContent);
  ...
  const commitRowsChange = (nextRows: ActionEditorRow[]) => {
    ...
    onContentChange(serializeActionEditorWriterRows(nextRows));
  };
  ...
  const requestFocusForInsertedEmptyRow = (currentRows: ActionEditorRow[], nextRows: ActionEditorRow[]) => {
    if (nextRows === currentRows) {
      return;
    }
    const currentRowIds = new Set(currentRows.map((row) => row.id));
    const insertedEmptyRowIndex = nextRows.findIndex(
      (row) =>
        !currentRowIds.has(row.id) &&
        (row.kind === 'dialogue' || row.kind === 'narration') &&
        row.text.length === 0,
    );
    if (insertedEmptyRowIndex === -1) {
      return;
    }
    emptyRowFocusTargetIndexRef.current = insertedEmptyRowIndex;
  ...
  const splitRowText = (rowId: string, selection: ActionEditorTextSelection) => {
    ...
    const nextRows = splitActionEditorTextRow(currentRows, rowId, selection);
    requestFocusForInsertedEmptyRow(currentRows, nextRows);
    commitRowsChange(nextRows);
  };

  const insertEmptyRowAfter = (rowId: string | null) => {
    const currentRows = draftRowsRef.current;
    const nextRows = insertActionEditorEmptyRowAfter(currentRows, rowId);
    requestFocusForInsertedEmptyRow(currentRows, nextRows);
    commitRowsChange(nextRows);
  };
  ...
  return (
    <div className="action-editor-writer" aria-label={t('actionEditor.writer.rows')}>
            ...
            return (
              <DialogueWriterRow
                ...
                onEmptyFocusRequestHandled={() => {
                  emptyRowFocusTargetIndexRef.current = null;
                }}
                pendingRowsRef={pendingRowsRef}
                onTextChange={(text) => updateRowText(row.id, text)}
                emptyFocusRequestId={rowIndex === emptyRowFocusTargetIndex ? emptyRowFocusRequestId : 0}
          ...
          if (row.kind === 'rawLine') {
            return (
              <RawLineWriterRow
                ...
                onActive={() => onActiveRowChange?.(row.id)}
                onCommitPending={flushPendingContent}
                onInsertAfter={() => insertEmptyRowAfter(row.id)}
                pendingRowsRef={pendingRowsRef}
                onTextChange={(text) => updateRowText(row.id, text)}
              />
            );
      ...
      <button className="action-editor-writer__add-row" onClick={() => insertEmptyRowAfter(null)} type="button">
        {t('actionEditor.writer.addRow')}
      </button>
    </div>
  );
});
...
const DialogueWriterRow = ({
  ...
  pendingRowsRef: MutableRefObject<ActionEditorRow[] | null>;
  speakerPool: string[];
}) => {
  const { t } = useTranslation();
  ...
  useEffect(() => {
    if (isFocused && pendingRowsRef.current !== null && row.text !== draftText) {
      return;
    }
  ...
  }, [draftText, isFocused, pendingRowsRef, row.id, row.text]);
  ...
  return (
    <div
      ...
      style={rowStyle}
    >
      <div className="action-editor-writer__speaker" role="group" aria-label={t('actionEditor.writer.speakerAria', { speaker: speakerLabel(row, t) })}>
        <button className="action-editor-writer__speaker-name" type="button">
          {rowIcon(row)}
          <span>{speakerLabel(row, t)}</span>
        </button>
        {row.kind === 'dialogue' ? (
          <>
            <input
              aria-describedby={imageAttributesHelpId}
              aria-label={t('actionEditor.writer.imageAttributesAria', { speaker: speakerLabel(row, t) })}
            ...
            <div className="action-editor-writer__speaker-attrs-help" id={imageAttributesHelpId} role="note">
              <strong>{t('actionEditor.writer.imageAttributesHelpTitle')}</strong>
              <span>
                {t('actionEditor.writer.imageAttributesHelpBefore')} <code>m 2d &quot;...&quot;</code>{' '}
                {t('actionEditor.writer.imageAttributesHelpAfter')}
              </span>
              <a
                ...
                target="_blank"
              >
                {t('actionEditor.writer.renpyDocs')}
              </a>
            </div>
          </>
        ) : null}
      ...
      <textarea
        aria-label={t('actionEditor.writer.dialogueAria', { speaker: speakerLabel(row, t) })}
...
const EmptyRowPicker = ({
  ...
  return (
    <div
      ...
      aria-label={t('actionEditor.writer.emptyPicker')}
    >
      {currentCardKind === 'dialogue' ? (
        <div className="action-editor-empty-picker__speaker-rail" aria-label={t('actionEditor.writer.speakerCycle')}>
      ...
      <div className="action-editor-empty-picker__card-belt" aria-label={t('actionEditor.writer.cardCycle')}>
        {actionEditorDraftCardKinds.map((kind, index) => (
          <span
            ...
            key={kind}
          >
            {kind === 'dialogue' ? t('actionEditor.writer.dialogue') : commandLabel(kind, kind, t)}
          </span>
        ))}
      </div>
    </div>
...
const InlineTextTagToolbar = ({
  ...
  return (
    <div
      ...
      aria-label={t('actionEditor.writer.toolbar.aria')}
      onKeyDown={handleKeyDown}
    >
      <div className="action-editor-toolbar__group" aria-label={t('actionEditor.writer.toolbar.style')}>
        <ToolbarButton label="B" onClick={() => onTag('bold')} title={t('actionEditor.writer.toolbar.bold')} />
        <ToolbarButton label="I" onClick={() => onTag('italic')} title={t('actionEditor.writer.toolbar.italic')} />
        <ToolbarButton label="U" onClick={() => onTag('underline')} title={t('actionEditor.writer.toolbar.underline')} />
        <ToolbarButton label="S" onClick={() => onTag('strikethrough')} title={t('actionEditor.writer.toolbar.strikethrough')} />
      </div>
      <div className="action-editor-toolbar__group" aria-label={t('actionEditor.writer.toolbar.parameters')}>
        <ToolbarMenuButton
          customInputLabel={t('actionEditor.writer.toolbar.customColorValue')}
          label={t('actionEditor.writer.toolbar.color')}
          menuLabel={t('actionEditor.writer.toolbar.colorOptions')}
          menuName="color"
          onMenuToggle={setOpenMenu}
          onSelect={(value) => onTag('color', { value })}
          openMenu={openMenu}
          options={toolbarMenuOptions.color}
          title={t('actionEditor.writer.toolbar.colorMenu')}
        />
        <ToolbarMenuButton
          customInputLabel={t('actionEditor.writer.toolbar.customSizeValue')}
          label={t('actionEditor.writer.toolbar.size')}
          menuLabel={t('actionEditor.writer.toolbar.sizeOptions')}
          menuName="size"
          onMenuToggle={setOpenMenu}
          onSelect={(value) => onTag('size', { value })}
          openMenu={openMenu}
          options={toolbarMenuOptions.size}
          title={t('actionEditor.writer.toolbar.sizeMenu')}
        />
        <ToolbarMenuButton
          customInputLabel={t('actionEditor.writer.toolbar.customCpsValue')}
          label="CPS"
          menuLabel={t('actionEditor.writer.toolbar.cpsOptions')}
          menuName="cps"
          onMenuToggle={setOpenMenu}
          onSelect={(value) => onTag('cps', { value })}
          openMenu={openMenu}
          options={toolbarMenuOptions.cps}
          title={t('actionEditor.writer.toolbar.cpsMenu')}
        />
      </div>
      <div className="action-editor-toolbar__group" aria-label={t('actionEditor.writer.toolbar.timing')}>
        <ToolbarButton label={t('actionEditor.writer.toolbar.pause')} onClick={() => onTag('pause')} title={t('actionEditor.writer.toolbar.pauseTag')} />
        <ToolbarButton label={t('actionEditor.writer.toolbar.wait')} onClick={() => onTag('wait')} title={t('actionEditor.writer.toolbar.waitTag')} />
        <ToolbarButton label={t('actionEditor.writer.toolbar.noWait')} onClick={() => onTag('noWait')} title={t('actionEditor.writer.toolbar.noWaitTag')} />
      </div>
    </div>
  );
};
...
const ToolbarMenuButton = ({
  ...
  const menuNoun = menuName === 'cps' ? 'CPS' : t(`actionEditor.writer.toolbar.${menuName}`);
      ...
      {isOpen ? (
        <div className="action-editor-toolbar__menu" id={menuId} role="menu" aria-label={menuLabel}>
          {options.map((option) => (
            ...
            >
              {menuName === 'color'
                ? option.label
                : menuName === 'size'
                  ? `${t('actionEditor.writer.toolbar.size')} ${option.value}`
                  : `CPS ${option.value}`}
            </button>
          ))}
          <label className="action-editor-toolbar__custom-option">
            <span>{t(`actionEditor.writer.toolbar.custom.${menuName}`)}</span>
            <input
          ...
          <button
            ...
            type="button"
          >
            {t('actionEditor.writer.toolbar.applyCustom', { name: menuNoun })}
          </button>
        </div>
      ) : null}
    </span>
...
const commandSupportsClause = (row: ActionEditorCommandRow, clauseName: ActionEditorCommandClauseName): boolean => {
  if (clauseName === 'at') {
    return row.kind === 'scene' || row.kind === 'show';
  }
...
const CommandClauseField = ({
  ...
  return (
    ...
    >
      ...
      <input
        aria-label={t('actionEditor.writer.clauseValue', { command: commandLabel, clause: name })}
        className={
          isFilled
...
const CommandOptionField = ({
  ...
  return (
    ...
    >
      <span className="action-editor-writer__command-clause-token">{name}</span>
      <input
        aria-label={t('actionEditor.writer.clauseValue', {
          command: commandLabel,
          clause: name === 'mods' ? t('actionEditor.writer.modifiers') : name,
        })}
        className={
          isFilled
            ? 'action-editor-writer__command-field action-editor-writer__command-field--filled'
...
const CommandWriterRow = ({
  ...
  const { t } = useTranslation();
  const label = commandLabel(row.kind, row.command, t);
  ...
  return (
    <div className={`action-editor-writer__row action-editor-writer__row--${row.kind}`}>
      ...
      <input
        aria-label={t('actionEditor.writer.primaryValue', { command: label })}
        className={
          primaryFilled
        ...
        placeholder={t('actionEditor.writer.namePlaceholder')}
        value={row.primary}
      />
      {suggestionsId ? (
      ...
      <button className="action-editor-writer__row-menu" onFocus={onActive} type="button" aria-label={t('actionEditor.writer.rowOptions', { type: label })}>
...
const RawLineWriterRow = ({
  ...
  return (
    <div className="action-editor-writer__row action-editor-writer__row--raw">
      <span className="action-editor-writer__command-label">[{t('actionEditor.writer.raw')}]</span>
      <input
        aria-label={t('actionEditor.writer.rawLine')}
