...
export interface ActionEditorTextSplice {
  index: number;
  deleteCount: number;
  insertText: string;
}

export const deriveActionEditorTextSplice = (
  before: string,
  after: string,
): ActionEditorTextSplice | null => {
  if (before === after) {
    return null;
  }
...
export interface ActionEditorPlaceholderClassification {
  canonical: boolean;
  comments: string[];
}

export const classifyActionEditorPlaceholder = (content: string): ActionEditorPlaceholderClassification => {
  ...
  for (const sourceLine of content.split(/\r?\n/)) {
    ...
    if (pass) {
      passCount += 1;
      if (pass[1]?.trim()) {
        comments.push(`# ${pass[1].trim()}`);
      }
      continue;
    }
    otherStatementCount += 1;
  }

  return { canonical: passCount === 1 && otherStatementCount === 0, comments };
};
...
export const projectActionEditorWriterRows = (content: string): ActionEditorRow[] => {
  const placeholder = classifyActionEditorPlaceholder(content);
  if (!placeholder.canonical) {
    return parseActionEditorContent(content);
  }

  const commentRows: ActionEditorRawLineRow[] = placeholder.comments.map((comment, index) => ({
    id: rowId(index),
    kind: 'rawLine',
    text: comment,
    source: comment,
...
export const serializeActionEditorWriterRows = (rows: ActionEditorRow[]): string => {
  const contentWithDraftRows = serializeActionEditorRows(rows);
  const committedRows = rows.filter(
    (row) => !((row.kind === 'dialogue' || row.kind === 'narration') && row.text.length === 0),
  );
  const content = serializeActionEditorRows(committedRows);
  const meaningfulLines = content.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
  const executableLines = meaningfulLines.filter((line) => !line.startsWith('#'));
  if (executableLines.length > 0) {
    return contentWithDraftRows;
  }
  return [...meaningfulLines.filter((line) => line.startsWith('#')), 'pass'].join('\n');
};
...
export const insertActionEditorEmptyRowAfter = (rows: ActionEditorRow[], rowIdToInsertAfter: string | null): ActionEditorRow[] => {
  const targetIndex =
    rowIdToInsertAfter === null ? rows.length - 1 : rows.findIndex((row) => row.id === rowIdToInsertAfter);
  if (rowIdToInsertAfter !== null && targetIndex === -1) {
    return rows;
  }
  ...
  if (
    nextExistingRow &&
    (nextExistingRow.kind === 'dialogue' || nextExistingRow.kind === 'narration') &&
    nextExistingRow.text.length === 0
  ) {
    return rows;
  }

  const baseRowId = rows[targetIndex]?.id ?? 'row-0000';
  const emptyRow: ActionEditorNarrationRow = {
    id: uniqueInsertedRowId(rows, baseRowId),
    kind: 'narration',
    speaker: 'Narrator',
    text: '',
  };

  return [...rows.slice(0, targetIndex + 1), emptyRow, ...rows.slice(targetIndex + 1)];
};
