...
describe('ActionEditorWriter Sprint 2 rows', () => {
  ...
  it('hides canonical pass, keeps its comment visible, and renders an empty writer row', () => {
    const html = renderToStaticMarkup(
      <ActionEditorWriter content={'# Add the cheese reveal later\npass'} onContentChange={vi.fn()} />,
    );

    expect(html).toContain('# Add the cheese reveal later');
    expect(html).not.toContain('value="pass"');
  ...
  it('renders scene at properties outside the scene image-name field', () => {
    const html = renderToStaticMarkup(
      <ActionEditorWriter
        content="scene art niva hysteria at bg_auto_cover with fade"
        onContentChange={vi.fn()}
      />,
    );

    expect(html).toContain('aria-label="Scene primary value"');
    expect(html).toContain('value="art niva hysteria"');
    expect(html).toContain('aria-label="Scene at value"');
    expect(html).toContain('value="bg_auto_cover"');
    expect(html).toContain('aria-label="Scene with value"');
    expect(html).toContain('value="fade"');
    expect(html).not.toContain('value="art niva hysteria at bg_auto_cover"');
  });
