...
import {
  ...
  classifyActionEditorPlaceholder,
...
describe('action editor content model', () => {
  ...
  it('derives one minimal text splice without replacing unrelated action lines', () => {
    const before = 'r "RenPy Mouse maps the attic."\nr "The cellar stays intact."';
    const after = 'r "RenPy Mouse carefully maps the attic."\nr "The cellar stays intact."';

    expect(deriveActionEditorTextSplice(before, after)).toEqual({
      index: 15,
      deleteCount: 0,
      insertText: 'carefully ',
    });
    expect(deriveActionEditorTextSplice(after, after)).toBeNull();
  });

  it('recognizes only one pass plus comments as the canonical placeholder', () => {
    expect(classifyActionEditorPlaceholder('pass')).toEqual({ canonical: true, comments: [] });
    expect(classifyActionEditorPlaceholder('# Later\npass')).toEqual({ canonical: true, comments: ['# Later'] });
    expect(classifyActionEditorPlaceholder('pass # Later')).toEqual({ canonical: true, comments: ['# Later'] });
    expect(classifyActionEditorPlaceholder('pass\npass').canonical).toBe(false);
    expect(classifyActionEditorPlaceholder('pass\nshow renpy mouse').canonical).toBe(false);
    expect(classifyActionEditorPlaceholder('# pass').canonical).toBe(false);
    expect(classifyActionEditorPlaceholder('"pass"').canonical).toBe(false);
  });

  it('projects canonical pass as comments plus one empty writer row', () => {
    const rows = projectActionEditorWriterRows('# Add animation later\npass');
    expect(rows.map((row) => row.kind)).toEqual(['rawLine', 'narration']);
    expect(rows[0]).toMatchObject({ text: '# Add animation later' });
    expect(rows[1]).toMatchObject({ kind: 'narration', text: '' });
  });

  it('restores pass after all executable rows are deleted but never rewrites non-canonical pass', () => {
    const placeholderRows = projectActionEditorWriterRows('# Keep note\npass');
    expect(serializeActionEditorWriterRows(placeholderRows)).toBe('# Keep note\npass');
    expect(serializeActionEditorWriterRows(parseActionEditorContent('pass\nshow renpy mouse'))).toBe(
      'pass\nshow renpy mouse',
    );
  });

  it('keeps a newly inserted empty row when real executable rows still exist', () => {
    const rows = parseActionEditorContent('m "RenPy Mouse waits."');
    const withEmpty = insertActionEditorEmptyRowAfter(rows, rows[0].id);
    expect(serializeActionEditorWriterRows(withEmpty)).toBe('m "RenPy Mouse waits."\n""');
  });
  ...
  it('keeps scene image name separate from RenPy at and with properties', () => {
    const rows = parseActionEditorContent('scene art niva hysteria at bg_auto_cover with fade');

    expect(rows[0]).toMatchObject({
      id: 'row-0000',
      kind: 'scene',
      primary: 'art niva hysteria',
      suffix: 'at bg_auto_cover with fade',
      clauses: { at: 'bg_auto_cover', with: 'fade' },
    });

    const renamedRows = updateActionEditorCommandPrimary(rows, 'row-0000', 'art niva neutral');
    const movedRows = updateActionEditorCommandClause(renamedRows, 'row-0000', 'at', 'renpy_mouse_bg_cover');

    expect(serializeActionEditorRows(movedRows)).toBe('scene art niva neutral at renpy_mouse_bg_cover with fade');
  });

  it('keeps show expression displayables out of ordinary image-name parsing while preserving layer properties', () => {
    const rows = parseActionEditorContent('show expression RainAnimation(drops=400) as rain onlayer master');

    expect(rows[0]).toMatchObject({
      id: 'row-0000',
      kind: 'show',
      primary: 'expression RainAnimation(drops=400) as rain',
      extraClauses: 'onlayer master',
      suffix: 'onlayer master',
    });
    expect(serializeActionEditorRows(rows)).toBe('show expression RainAnimation(drops=400) as rain onlayer master');
  });
