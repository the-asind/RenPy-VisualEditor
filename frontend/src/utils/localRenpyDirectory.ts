...
export type RenpyImageDefinition =
  | {
      kind: 'single';
      path: string;
    }
  | {
      ...
      layers: Array<{ x: number; y: number; path: string }>;
    };
...
export interface LocalWriteManifest {
  changed: string[];
  created: string[];
  unchanged: string[];
  missingFromExport: string[];
  skippedNonRpy: string[];
}
...
const addImageDefinition = (
  imageDefinitions: Record<string, RenpyImageDefinition>,
  imageName: string,
  definition: RenpyImageDefinition | null,
): void => {
  const normalizedImageName = imageName.trim().replace(/\s+/g, ' ');
  if (normalizedImageName && definition) {
    imageDefinitions[normalizedImageName] = definition;
  }
};
...
const startsImageStatement = (line: string): boolean => /^\s*image\s+.+?\s*=/.test(line);

const findImageEntryByRenpyDisplayablePath = (
  entriesByPath: Map<string, ProjectAssetCatalogEntry>,
  rawPath: string,
): ProjectAssetCatalogEntry | undefined => {
  const strippedPath = stripRenpyPathDirectives(rawPath);
  return (
    entriesByPath.get(normalizeCatalogPath(strippedPath)) ??
    (!/^images\//i.test(strippedPath)
      ? entriesByPath.get(normalizeCatalogPath(`images/${strippedPath}`))
      : undefined)
  );
};

const parseCompositeDefinition = (
  expression: string,
  entriesByPath: Map<string, ProjectAssetCatalogEntry>,
): RenpyImageDefinition | null => {
  const compositeMatch = expression.match(/\bComposite\s*\(([\s\S]*)\)\s*$/);
  if (!compositeMatch) {
    return null;
  }
  const body = compositeMatch[1];
  const sizeMatch = body.match(/\(\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)\s*\)/);
  if (!sizeMatch) {
    return null;
  }
  ...
  const layers: Array<{ x: number; y: number; path: string }> = [];
  const layerPattern = /\(\s*(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)\s*\)\s*,\s*(["'])((?:\\.|(?!\3).)*)\3/g;
  let layerMatch: RegExpExecArray | null;
  while ((layerMatch = layerPattern.exec(body)) !== null) {
    const x = Number.parseFloat(layerMatch[1]);
    const y = Number.parseFloat(layerMatch[2]);
    const path = stripRenpyPathDirectives(layerMatch[4].replace(/\\(["'])/g, '$1'));
    const entry = findImageEntryByRenpyDisplayablePath(entriesByPath, path);
    if (Number.isFinite(x) && Number.isFinite(y) && entry?.kind === 'image') {
      layers.push({ x, y, path: entry.path });
    }
  }
...
const parseSingleImageDefinition = (
  expression: string,
  entriesByPath: Map<string, ProjectAssetCatalogEntry>,
): RenpyImageDefinition | null => {
  const imagePaths = extractQuotedStrings(expression)
    .map((quotedPath) => findImageEntryByRenpyDisplayablePath(entriesByPath, quotedPath))
    .filter((entry): entry is ProjectAssetCatalogEntry => entry?.kind === 'image');
  const uniquePaths = Array.from(new Set(imagePaths.map((entry) => entry.path)));
  return uniquePaths.length === 1 ? { kind: 'single', path: uniquePaths[0] } : null;
};
...
const attachRenpyAliasesFromScript = async (
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (imageMatch) {
      ...
      let parenthesesBalance = countParenthesesDelta(imageMatch[2]);
      for (
        let continuationIndex = index + 1;
        parenthesesBalance > 0 && continuationIndex < lines.length;
        continuationIndex += 1
        ...
        const continuationLine = lines[continuationIndex];
        if (startsImageStatement(continuationLine)) {
          break;
        }
      ...
      const compositeDefinition = parseCompositeDefinition(expression, entriesByPath);
      if (compositeDefinition) {
        addImageDefinition(imageDefinitions, imageName, compositeDefinition);
        continue;
      }
      const singleDefinition = parseSingleImageDefinition(expression, entriesByPath);
      addImageDefinition(imageDefinitions, imageName, singleDefinition);
      for (const quotedPath of extractQuotedStrings(expression)) {
        const entry = findImageEntryByRenpyDisplayablePath(entriesByPath, quotedPath);
        if (entry?.kind === 'image') {
...
const attachRenpyAliases = async (
  entries: ProjectAssetCatalogEntry[],
  rpyFiles: LocalRenpyScriptFile[],
): Promise<{ characterImages: Record<string, string>; imageDefinitions: Record<string, RenpyImageDefinition> }> => {
  const entriesByPath = new Map(entries.map((entry) => [normalizeCatalogPath(entry.path), entry]));
  const characterImages: Record<string, string> = {};
  const imageDefinitions: Record<string, RenpyImageDefinition> = {};
  await Promise.all(
    rpyFiles.map((script) => attachRenpyAliasesFromScript(script, entriesByPath, characterImages, imageDefinitions)),
  );
  return { characterImages, imageDefinitions };
};
...
export const scanLocalRenpyGameDirectory = async (
  ...
  const { characterImages, imageDefinitions } = await attachRenpyAliases(entries, rpyFiles);
  ...
  return {
    catalog: {
      ...
      ...(Object.keys(imageDefinitions).length ? { imageDefinitions } : {}),
      entries,
    },
    rpyFiles,
  };
...
export const buildLocalWriteManifest = async (
  session: LocalRenpyGameDirectorySession,
  exportedFiles: Record<string, string>,
): Promise<LocalWriteManifest> => {
  const scan = await session.scan();
  const localScripts = new Map(scan.rpyFiles.map((script) => [normalizeCatalogPath(script.path), script]));
  const exportedScriptPaths = new Set<string>();
  const manifest: LocalWriteManifest = {
    changed: [],
    created: [],
    unchanged: [],
    missingFromExport: [],
  ...
  for (const [path, content] of Object.entries(exportedFiles)) {
    const safePath = normalizeRelativePath(path);
    if (!safePath || extensionForPath(safePath) !== '.rpy') {
      manifest.skippedNonRpy.push(path);
      continue;
    }
    exportedScriptPaths.add(normalizeCatalogPath(safePath));
    const localScript = localScripts.get(normalizeCatalogPath(safePath));
    if (!localScript) {
      manifest.created.push(safePath);
      continue;
    }
    const localContent = await localScript.file.text();
    if (localContent === content) {
      manifest.unchanged.push(safePath);
    } else {
      manifest.changed.push(safePath);
    }
  }

  for (const script of scan.rpyFiles) {
    if (!exportedScriptPaths.has(normalizeCatalogPath(script.path))) {
      manifest.missingFromExport.push(script.path);
    }
  }

  manifest.changed.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
  manifest.created.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
  manifest.unchanged.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
  manifest.missingFromExport.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
  manifest.skippedNonRpy.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
  return manifest;
};

export const formatLocalWriteManifestConfirmation = (manifest: LocalWriteManifest): string => {
  const lines = [
    'Write exported .rpy files to the connected local game folder?',
    `Changed: ${manifest.changed.length}`,
    `Created: ${manifest.created.length}`,
    `Unchanged: ${manifest.unchanged.length}`,
    `Local .rpy not in export: ${manifest.missingFromExport.length}`,
    `Skipped non-.rpy export entries: ${manifest.skippedNonRpy.length}`,
  ];
  const changedPreview = [...manifest.changed, ...manifest.created].slice(0, 12);
  if (changedPreview.length) {
    lines.push('', ...changedPreview.map((path) => `- ${path}`));
  }
  if (manifest.missingFromExport.length) {
    lines.push('', 'No local files will be deleted.');
  }
  return lines.join('\n');
};
