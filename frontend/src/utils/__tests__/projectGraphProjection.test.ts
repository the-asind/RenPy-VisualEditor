...
import {
  ...
  buildProjectGraphRelationNavigationKeyframes,
  ...
  getProjectGraphRelationEdgeVisibility,
...
describe('ProjectGraph code-only and flow-control projection', () => {
  it('projects technical or label-free files as code-only frames with source preview text', () => {
    const graph: ProjectGraphSnapshot = {
      project_id: 'code-only-projection-project',
      files: [
        {
          id: 'file-gui',
          path: 'gui.rpy',
          order: '0000',
          visual: { position: { x: 0, y: 0 }, size: { width: 720, height: 420 } },
      ...
      labels: [
        {
          ...
          scope: 'global',
          label_start_node_id: 'start-hidden',
          source_span: { start_line: 2, end_line: 2 },
          visual: { position: { x: 48, y: 48 }, size: { width: 420, height: 220 } },
        },
      ],
      label_starts: [
        {
    ...
    expect(projection.nodes[0]).toMatchObject({
      ...
      data: {
        codeOnlyFile: true,
        codeOnlyReason: 'renpy_template',
        sourceContent: 'init python:\n    gui.init(1920, 1080)\n',
      },
    });
  });

  it('projects jump, call, and return as header-height flow-control nodes', () => {
    const graph: ProjectGraphSnapshot = {
      project_id: 'flow-control-projection-project',
      files: [
        {
          id: 'file-main',
          path: 'script.rpy',
      ...
      nodes: ['jump', 'call', 'return'].map((type, index) => ({
        id: `node-${type}`,
        file_id: 'file-main',
        label_id: 'label-start',
        parent_node_id: null,
        type,
        content: type === 'return' ? 'return' : `${type} next_scene`,
        order: `${index}`.padStart(4, '0'),
        source_span: { start_line: index + 1, end_line: index + 1 },
        metadata: {},
        visual: { position: { x: 96, y: 136 + index * 112 }, size: { width: 320, height: 88 } },
      })),
      edges: [],
      diagnostics: [],
      source_index: { files: {} },
    ...
    const projection = projectGraphToReactFlow(graph);
    const flowControls = projection.nodes.filter((node) => node.type === 'scenarioNode');

    expect(flowControls).toHaveLength(3);
    for (const node of flowControls) {
      expect(node.height).toBeLessThanOrEqual(44);
      expect(node.data).toMatchObject({ visualRole: 'flowControl' });
    }
  });
});
...
describe('relation-aware label frame packing helpers', () => {
  ...
  it('hides dense jump and call relation edges until a jump or call node is selected', () => {
    const relationEdges = Array.from({ length: 21 }, (_, index) => ({
      id: `relation-${index}`,
      source: `node-jump-${index}`,
      target: `start-${index}`,
      data: { kind: index % 2 === 0 ? 'jump' : 'call' },
    }));
    const edges = [
      { id: 'derived-sequence', source: 'a', target: 'b', data: { kind: 'sequence', derived: true } },
      ...relationEdges,
    ];

    expect(getProjectGraphRelationEdgeVisibility(edges as never, null).map((edge) => edge.id)).toEqual([
      'derived-sequence',
    ]);
    expect(getProjectGraphRelationEdgeVisibility(edges as never, 'node-jump-4').map((edge) => edge.id)).toEqual([
      'derived-sequence',
      'relation-4',
    ]);
    expect(getProjectGraphRelationEdgeVisibility(edges.slice(0, 21) as never, null)).toHaveLength(21);
  });
  ...
  it('builds relation navigation keyframes with a midpoint zoom-out and original final zoom', () => {
    const keyframes = buildProjectGraphRelationNavigationKeyframes({
      canvasSize: { width: 1000, height: 600 },
      fromViewport: { x: -100, y: -80, zoom: 1.2 },
      targetCenter: { x: 900, y: 480 },
      maxZoom: 1.3,
    });

    expect(keyframes).toHaveLength(3);
    expect(keyframes[0]).toMatchObject({ x: -100, y: -80, zoom: 1.2 });
    expect(keyframes[1].zoom).toBeLessThan(1.2);
    expect(keyframes[2].zoom).toBe(1.2);
    expect(keyframes[2].x).toBeCloseTo(500 - 900 * 1.2, 4);
    expect(keyframes[2].y).toBeCloseTo(300 - 480 * 1.2, 4);

    const cappedKeyframes = buildProjectGraphRelationNavigationKeyframes({
      canvasSize: { width: 1000, height: 600 },
      fromViewport: { x: -100, y: -80, zoom: 2.45 },
      targetCenter: { x: 900, y: 480 },
      maxZoom: 2.5,
    });

    expect(cappedKeyframes[1].zoom).toBeLessThan(2.45);
    expect(cappedKeyframes[2].zoom).toBe(2.45);
    expect(cappedKeyframes.every((frame) => frame.zoom <= 2.5)).toBe(true);
  });
...
const makeRootFileCalendarGraph = (fileCount = 12, manual = false): ProjectGraphSnapshot => {
  const files = Array.from({ length: fileCount }, (_, index) => ({
    id: `file-root-${index}`,
    path: `chapters/renpy_mouse_day_${String(index + 1).padStart(2, '0')}.rpy`,
    order: `${index}`.padStart(4, '0'),
    visual: {
      position: manual
        ? { x: 80 + (index % 3) * 760, y: 60 + Math.floor(index / 3) * 520 }
        : { x: index * 760, y: 0 },
      size: { width: 640, height: 360 },
    },
    metadata: manual ? { _manual_position: true } : undefined,
  }));

  return {
    project_id: 'root-file-calendar-project',
    files,
    labels: [],
    label_starts: [],
    nodes: [],
    ...
    source_index: {
      files: Object.fromEntries(
        files.map((file) => [
          ...
          {
            path: file.path,
            content: '# RenPy Mouse stores a small technical chapter here.\n',
          },
        ]),
      ),
    },
...
describe('projectGraphToReactFlow static projection', () => {
  ...
  it('splices a source-span-less menu into durable narrative order before an imported terminal tail', () => {
    const spliceGraph: ProjectGraphSnapshot = {
      ...graph,
      nodes: [
        {
          ...graph.nodes[0],
          id: 'node-action',
          type: 'action',
          content: 'r "RenPy Mouse chooses where the new branch belongs."',
          order: '0000',
          source_span: { start_line: 1, end_line: 1 },
        },
        {
          ...
          parent_node_id: null,
          type: 'call',
          content: 'call discuss_cheese',
          order: '0002',
          source_span: { start_line: 2, end_line: 2 },
          metadata: {},
          visual: { position: { x: 96, y: 248 }, size: { width: 320, height: 44 } },
        },
        {
          id: 'node-jump-tail',
          file_id: 'file-day-1',
          ...
          parent_node_id: null,
          type: 'jump',
          content: 'jump cheese_finale',
          order: '0003',
          source_span: { start_line: 3, end_line: 3 },
          metadata: {},
          visual: { position: { x: 96, y: 336 }, size: { width: 320, height: 44 } },
        },
        {
          id: 'node-menu-inserted',
          file_id: 'file-day-1',
          label_id: 'label-start',
          parent_node_id: null,
          type: 'menu',
          ...
          source_span: null,
          metadata: {},
          visual: { position: { x: 96, y: 224 }, size: { width: 320, height: 120 } },
        },
        {
          id: 'node-menu-choice',
          file_id: 'file-day-1',
          label_id: 'label-start',
          parent_node_id: 'node-menu-inserted',
          type: 'menu_choice',
          content: '"Follow the cheese trail":',
          order: '0001.0000',
          source_span: null,
          metadata: {},
          visual: { position: { x: 96, y: 376 }, size: { width: 320, height: 72 } },
        },
        {
          id: 'node-menu-action',
          file_id: 'file-day-1',
          ...
          source_span: null,
          metadata: {},
          visual: { position: { x: 96, y: 480 }, size: { width: 320, height: 88 } },
        },
      ],
    };

    const projection = projectGraphToReactFlow(spliceGraph);
    ...
    expect(derivedPairs).toEqual(
      new Set([
        'start-node-start->node-action',
        'node-action->node-menu-inserted',
        'node-menu-inserted->node-menu-choice',
        'node-menu-choice->node-menu-action',
        'node-menu-action->node-call-tail',
        'node-call-tail->node-jump-tail',
      ]),
    );
    expect(derivedPairs.has('node-action->node-call-tail')).toBe(false);
  });
  ...
  it('wraps import-like root file frames into a calendar-shaped project map', () => {
    const calendarGraph = makeRootFileCalendarGraph(12);
    const projection = projectGraphToReactFlow(calendarGraph);
    const byId = new Map(projection.nodes.map((node) => [node.id, node]));
    const files = calendarGraph.files.map((file) => byId.get(file.id)!);
    const distinctRows = distinctRoundedValues(files.map((file) => file.position.y));
    const distinctColumns = distinctRoundedValues(files.map(nodeCenterX));
    const minX = Math.min(...files.map((file) => file.position.x));
    const maxX = Math.max(...files.map((file) => file.position.x + Number(file.width)));
    const minY = Math.min(...files.map((file) => file.position.y));
    const maxY = Math.max(...files.map((file) => file.position.y + Number(file.height)));
    ...
    for (let leftIndex = 0; leftIndex < files.length; leftIndex += 1) {
      for (let rightIndex = leftIndex + 1; rightIndex < files.length; rightIndex += 1) {
        expect(rectsOverlap(nodeRect(files[leftIndex]), nodeRect(files[rightIndex]))).toBe(false);
      }
    }
  });

  it('preserves existing manual 2D root file frame layouts', () => {
    const manualGraph = makeRootFileCalendarGraph(6, true);
    const projection = projectGraphToReactFlow(manualGraph);
    const byId = new Map(projection.nodes.map((node) => [node.id, node]));

    for (const file of manualGraph.files) {
      const projected = byId.get(file.id)!;
      expect(projected.position).toEqual(file.visual.position);
    }
  });
