...
import {
  ...
  openClockworkLibraryDemoProject,
...
describe('createProjectSessionToken', () => {
  ...
  it('requests a project-scoped collaboration session token', async () => {
    mock.onPost('/auth/session-token').reply((config) => {
      expect(JSON.parse(config.data)).toEqual({ project_id: 'project-session' });
      return [200, { access_token: 'session-token', token_type: 'bearer' }];
    });

    await expect(createProjectSessionToken('project-session')).resolves.toEqual({
...
describe('ProjectGraph snapshot loading', () => {
  ...
  it('opens the server-hosted Clockwork Library demo project', async () => {
    const importResult: ProjectGraphImportResult = {
      project_id: 'demo-clockwork-copy',
      file_count: 8,
      label_count: 12,
      label_start_count: 12,
      node_count: 36,
      edge_count: 10,
      diagnostics: {
        total: 0,
    ...
    mock.onPost('/projects/demo/clockwork-library').reply(200, importResult);

    await expect(openClockworkLibraryDemoProject()).resolves.toEqual(importResult);
  });

  it('loads the public Clockwork Library demo preview graph for the landing page', async () => {
    const preview = {
      project_id: 'clockwork-library-demo-preview',
      graph: {
        project_id: 'clockwork-library-demo-preview',
        files: [
          {
    ...
    mock.onGet('/projects/demo/clockwork-library/preview').reply(200, preview);

    await expect(loadClockworkLibraryDemoPreview()).resolves.toEqual(preview);
  });
  ...
  it('commits a ProjectGraph continuation command and decodes authoritative update headers', async () => {
    const update = new Uint8Array([9, 8, 7, 6]);
    const result = {
      createdNodeIds: ['node-jump'],
      selectedNodeId: 'node-jump',
      createdTargetIds: { files: ['file-cheese'], labels: ['label-cheese'], labelStarts: ['start-cheese'] },
    };
    const resultHeader = btoa(JSON.stringify(result));

    mock.onPost('/projects/project-load-snapshot/continuation-commands').reply((config) => {
      expect(config.responseType).toBe('arraybuffer');
      ...
      return [
        ...
        {
          'x-project-graph-revision': '4',
          'x-project-graph-command-result': resultHeader,
    ...
    await expect(
      commitProjectGraphContinuation('project-load-snapshot', {
        baseRevision: 3,
        sourceNodeId: 'node-intro',
        action: 'jump',
        target: {
  ...
  it('commits an authoritative ProjectGraph structure command', async () => {
    const update = new Uint8Array([4, 3, 2, 1]);
    const result = {
      selectedEntityId: 'label-cheese',
      createdEntityIds: { files: [], labels: ['label-cheese'], labelStarts: ['start-cheese'], nodes: ['node-pass'] },
    };
    mock.onPost('/projects/project-load-snapshot/structure-commands').reply((config) => {
      expect(config.responseType).toBe('arraybuffer');
      expect(JSON.parse(config.data as string)).toEqual({ kind: 'label', fileId: 'file-main', name: 'cheese' });
      return [200, update.buffer, {
        'x-project-graph-revision': '5',
        'x-project-graph-command-result': btoa(JSON.stringify(result)),
      }];
    });

    await expect(commitProjectGraphStructure('project-load-snapshot', {
      kind: 'label', fileId: 'file-main', name: 'cheese',
    })).resolves.toEqual({ update, revision: 5, result });
  });
