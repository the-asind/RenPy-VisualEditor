...
def test_project_graph_import_route_preserves_directory_paths_and_catalog(client, project_owner):
    ...
    assert catalog_payload["catalog"]["imageDefinitions"] == asset_catalog["imageDefinitions"]


def test_clockwork_library_demo_project_creates_action_showcase_graph_and_server_asset_catalog(client, project_owner):
    response = client.post("/api/projects/demo/clockwork-library")

    assert response.status_code == 200
    payload = response.json()
    assert payload["project_id"] != project_owner["project_id"]
    assert payload["file_count"] >= 7
    assert payload["label_count"] == 8
    assert payload["node_count"] >= 30
    assert payload["edge_count"] == 2
    assert payload["snapshot_available"] is True
    assert payload["catalog_entry_count"] >= 6

    project_response = client.get(f"/api/projects/{payload['project_id']}")
    assert project_response.status_code == 200
    assert project_response.json()["description"] == (
        "Action-focused Ren'Py demo with rich dialogue, images, and audio."
    )

    snapshot_response = client.get(f"/api/projects/{payload['project_id']}/graph-snapshot")
    ...
    options_file = next(file for file in restored_graph["files"] if file["path"] == "options.rpy")
    assert options_file["metadata"] == {
        "code_only": True,
        "code_only_reason": "renpy_template",
    }
    assert {label["qualified_name"] for label in restored_graph["labels"]} >= {
        "start",
        "library_hall",
        "library_hall.secret_shelf",
        "basement_clock",
        "basement_clock.tune_gears",
        "rooftop_signal",
        "ending_bright",
        "ending_quiet",
    }

    catalog_response = client.get(f"/api/projects/{payload['project_id']}/asset-catalog")
    assert catalog_response.status_code == 200
    catalog = catalog_response.json()["catalog"]
    assert catalog["root_kind"] == "renpy-game-root"
    assert catalog["game_directory"] == "game"
    assert catalog["characterImages"] == {"r": "renpy"}
    assert catalog["imageDefinitions"]["library night"]["kind"] == "single"
    assert catalog["imageDefinitions"]["renpy bright"]["kind"] == "single"
    urls = [entry.get("url") for entry in catalog["entries"] if entry["kind"] in {"image", "audio"}]
    assert urls
    assert all(url and url.startswith("/demo-assets/clockwork-library/v1/") for url in urls)


def test_owned_project_quota_blocks_regular_and_demo_project_creation(
    client,
    project_owner,
    temp_database,
    monkeypatch,
):
    """The existing owned project consumes the only available project slot."""
    monkeypatch.setattr(projects, "MAX_OWNED_PROJECTS_PER_USER", 1, raising=False)

    regular_response = client.post(
        "/api/projects/",
        json={"name": "One Project Too Many", "description": "RenPy mouse overload"},
    )
    demo_response = client.post("/api/projects/demo/clockwork-library")

    for response in (regular_response, demo_response):
        assert response.status_code == 409
        assert response.json()["detail"] == {
            "code": "owned_project_quota_exceeded",
            "limit": 1,
        }

    owned_projects = temp_database.get_user_projects(project_owner["id"])
    assert [project["id"] for project in owned_projects] == [project_owner["project_id"]]


def test_clockwork_library_demo_preview_returns_project_graph_source_of_truth(client):
    response = client.get("/api/projects/demo/clockwork-library/preview")

    assert response.status_code == 200
    payload = response.json()
    assert payload["project_id"] == "clockwork-library-demo-preview"
    ...
    assert {label["qualified_name"] for label in graph["labels"]} >= {
        "start",
        "library_hall",
        "library_hall.secret_shelf",
        "basement_clock",
    ...
    assert any(node["type"] == "menu_choice" for node in graph["nodes"])
    assert any(edge["kind"] == "jump" for edge in graph["edges"])
    assert any(edge["kind"] == "call" for edge in graph["edges"])

    options_file = next(file for file in graph["files"] if file["path"] == "options.rpy")
    assert options_file["metadata"] == {
        "code_only": True,
        "code_only_reason": "renpy_template",
    }
    assert graph["source_index"]["files"][options_file["id"]]["path"] == "options.rpy"

    catalog = payload["asset_catalog"]
    assert catalog["imageDefinitions"]["library night"]["path"] == "images/backgrounds/library-night.webp"
    assert catalog["imageDefinitions"]["renpy bright"]["path"] == "images/renpy/bright.png"
    assert any(entry.get("url") == "/demo-assets/clockwork-library/v1/audio/music/library-loop.wav" for entry in catalog["entries"])


def test_clockwork_library_demo_preview_does_not_build_a_loro_snapshot(client, monkeypatch):
    def fail_if_snapshot_bridge_runs(*_args, **_kwargs):
        raise AssertionError("Anonymous demo preview must not start the Node/Loro snapshot bridge")

    monkeypatch.setattr(
        projects.ProjectGraphCrdtSnapshotBridge,
        "export_snapshot",
        fail_if_snapshot_bridge_runs,
    )

    response = client.get("/api/projects/demo/clockwork-library/preview")

    assert response.status_code == 200
    assert response.json()["project_id"] == "clockwork-library-demo-preview"
    assert response.headers["cache-control"] == "public, max-age=300, stale-while-revalidate=3600"


def test_clockwork_library_demo_preview_never_builds_the_graph_even_on_cold_cache(client, monkeypatch):
    projects._clockwork_library_demo_preview_payload.cache_clear()
    original_import_files = projects.ProjectGraphImporter.import_files
    import_calls = 0

    def count_imports(importer, *args, **kwargs):
        nonlocal import_calls
        import_calls += 1
        return original_import_files(importer, *args, **kwargs)

    monkeypatch.setattr(projects.ProjectGraphImporter, "import_files", count_imports)
    try:
        first_response = client.get("/api/projects/demo/clockwork-library/preview")
        second_response = client.get("/api/projects/demo/clockwork-library/preview")
    finally:
        projects._clockwork_library_demo_preview_payload.cache_clear()
...
def test_clockwork_library_demo_keeps_one_of_each_relation_statement(client):
    response = client.get("/api/projects/demo/clockwork-library/preview")

    assert response.status_code == 200
    graph = response.json()["graph"]
    call_nodes = [node for node in graph["nodes"] if node["type"] == "call"]
    jump_nodes = [node for node in graph["nodes"] if node["type"] == "jump"]
    dynamic_jump_nodes = [
        node
        for node in jump_nodes
        if node["content"].strip().startswith("jump expression ")
    ]
    direct_jump_nodes = [node for node in jump_nodes if node not in dynamic_jump_nodes]

    assert len(call_nodes) == 1
    assert len(direct_jump_nodes) == 1
    assert len(dynamic_jump_nodes) == 1
    assert [edge["kind"] for edge in graph["edges"]].count("call") == 1
    assert [edge["kind"] for edge in graph["edges"]].count("jump") == 1
    assert [diagnostic["code"] for diagnostic in graph["diagnostics"]].count("dynamic_target") == 1


def test_clockwork_library_demo_action_nodes_are_dialogue_rich(client):
    response = client.get("/api/projects/demo/clockwork-library/preview")

    assert response.status_code == 200
    graph = response.json()["graph"]
    action_nodes = [node for node in graph["nodes"] if node["type"] == "action"]
    assert action_nodes
    for node in action_nodes:
        ...
        assert len(spoken_lines) >= 4, node["content"]

    labels_by_name = {label["qualified_name"]: label for label in graph["labels"]}
    for qualified_name in {
        ...
        label = labels_by_name[qualified_name]
        first_action = next(
            node
            for node in action_nodes
            if node["label_id"] == label["id"]
        )
        spoken_lines = [
            line
            for line in first_action["content"].splitlines()
            if line.lstrip().startswith('r "')
        ]
        assert len(spoken_lines) >= 6, qualified_name


def test_project_graph_import_route_marks_code_only_files_in_snapshot(client, project_owner):
    response = client.post(
        f"/api/projects/{project_owner['project_id']}/graph-import",
        files=[
            (
                "files",
                (
    ...
    assert restored_graph["files"][0]["path"] == "gui.rpy"
    assert restored_graph["files"][0]["metadata"] == {
        "code_only": True,
        "code_only_reason": "renpy_template",
    }
    assert restored_graph["labels"] == []
    assert restored_graph["nodes"] == []
    assert restored_graph["source_index"]["files"][restored_graph["files"][0]["id"]]["content"].startswith("init python:")
...
def test_project_graph_import_route_rejects_too_many_files(client, project_owner, monkeypatch):
    """Import must reject too many uploaded files before parsing or bridge work."""
    monkeypatch.setattr(projects, "MAX_GRAPH_IMPORT_FILES", 1, raising=False)
...
def test_project_graph_import_route_rejects_oversized_asset_catalog(client, project_owner, monkeypatch):
    """Local-directory import must cap attacker-controlled asset catalog JSON."""
    monkeypatch.setattr(projects, "MAX_ASSET_CATALOG_BYTES", 64, raising=False)
    oversized_catalog = {
        "root_kind": "renpy-game-root",
        "game_directory": "game",
        "entries": [
            {
                "path": "images/renpy/private-large-catalog.png",
                "name": "private-large-catalog.png",
                "extension": ".png",
                "kind": "image",
                "size": 1,
                "lastModified": 1,
    ...
    response = _upload_directory_import(
        client,
        project_owner["project_id"],
        [("renpy_mouse_day_1.rpy", "renpy_mouse_day_1.rpy")],
        oversized_catalog,
    )
