...
def test_project_crdt_snapshot_record_tracks_revision(temp_database):
    project_id = create_project(temp_database)

    temp_database.save_project_crdt_snapshot(project_id, b"\x00loro-snapshot-v1")
    first_record = temp_database.get_project_crdt_snapshot_record(project_id)
    temp_database.save_project_crdt_snapshot(project_id, b"\x00loro-snapshot-v2")
    second_record = temp_database.get_project_crdt_snapshot_record(project_id)

    assert first_record["snapshot"] == b"\x00loro-snapshot-v1"
    assert first_record["revision"] == 1
    assert second_record["snapshot"] == b"\x00loro-snapshot-v2"
    assert second_record["revision"] == 2


def test_project_crdt_snapshot_compare_and_swap_rejects_stale_revision(temp_database):
    project_id = create_project(temp_database)
    temp_database.save_project_crdt_snapshot(project_id, b"\x00loro-snapshot-v1")

    assert temp_database.compare_and_swap_project_crdt_snapshot(project_id, b"\x00loro-snapshot-v2", expected_revision=1)
    assert not temp_database.compare_and_swap_project_crdt_snapshot(project_id, b"\x00stale", expected_revision=1)

    record = temp_database.get_project_crdt_snapshot_record(project_id)
    assert record["snapshot"] == b"\x00loro-snapshot-v2"
...
def test_database_initialization_adds_missing_crdt_snapshot_revision_column(temp_database):
    with temp_database._get_connection() as conn:
        conn.execute("DROP TABLE IF EXISTS project_crdt_snapshots")
        conn.execute(
            '''
            CREATE TABLE project_crdt_snapshots (
    ...
    with migrated._get_connection() as conn:
        columns = {row["name"] for row in conn.execute("PRAGMA table_info(project_crdt_snapshots)").fetchall()}
