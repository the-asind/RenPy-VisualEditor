...
def test_import_marks_renpy_template_and_label_free_files_as_code_only(tmp_path):
    ...
    definitions_file = tmp_path / "definitions.rpy"
    definitions_file.write_text(
        "\n".join(
            [
                'define r = Character("RenPy Mouse")',
                'image cheese = "cheese.png"',
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    graph = ProjectGraphImporter().import_files(
        project_id="mouse-renpy-code-only-project",
        files=[gui_file, definitions_file],
    )

    files_by_path = {file.path: file for file in graph.files}
    code_only_file_ids = {files_by_path["gui.rpy"].id, files_by_path["definitions.rpy"].id}

    assert files_by_path["gui.rpy"].metadata == {
        "code_only": True,
        "code_only_reason": "renpy_template",
    }
    assert files_by_path["definitions.rpy"].metadata == {
        "code_only": True,
        "code_only_reason": "no_labels",
    }
    assert all(label.file_id not in code_only_file_ids for label in graph.labels)
    assert all(node.file_id not in code_only_file_ids for node in graph.nodes)
    assert graph.source_index["files"][files_by_path["gui.rpy"].id]["content"] == gui_file.read_text(encoding="utf-8")
