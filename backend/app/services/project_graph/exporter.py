...
class ProjectGraphExporter:
    ...
    def export(self, graph: ProjectGraph) -> dict[str, str]:
        ...
        for file in sorted(graph.files, key=lambda item: item.order):
            if self._is_code_only_file(file):
                exported[file.path] = self._source_file_content(graph, file)
                continue
            ...
            for label in labels:
                ...
                if not top_level_nodes:
                    lines.append(f"{self.LABEL_INDENT}pass")
    ...
    @staticmethod
    def _is_code_only_file(file: FileFrame) -> bool:
        return file.metadata.get("code_only") is True

    @staticmethod
    def _source_file_content(graph: ProjectGraph, file: FileFrame) -> str:
