...
class ProjectGraphImporter:
    ...
    RENPY_TEMPLATE_CODE_ONLY_FILES = {
        "gui.rpy",
        "options.rpy",
        "screens.rpy",
        "settings.rpy",
    ...
    def import_files(
        ...
        for index, path in enumerate(paths):
            ...
            is_renpy_template_file = self._is_renpy_template_code_file(display_path)
            labels, starts = ([], []) if is_renpy_template_file else self._scan_labels(file_id=file_id, content=content)
            code_only_reason = "renpy_template" if is_renpy_template_file else "no_labels" if not labels else None
            file_metadata = (
                {
                    "code_only": True,
                    "code_only_reason": code_only_reason,
                }
                if code_only_reason
                else {}
            )
            ...
            statement_nodes = (
                []
                if code_only_reason
                else self._scan_statement_nodes(
                    file_id=file_id,
                    content=content,
                    labels=labels,
                )
    ...
    def _is_renpy_template_code_file(self, display_path: str) -> bool:
        filename = re.split(r"[/\\]", display_path.strip())[-1].lower()
        return filename in self.RENPY_TEMPLATE_CODE_ONLY_FILES
