...
13. Derived локальный flow использует ортогональную routing модель, чтобы линии читались как дерево, а не как произвольные кривые. Forward `sequence` edges могут использовать встроенный `step`; если source и target уже стоят на одной вертикальной оси, projection использует `straight`, чтобы не создавать лишнюю ступеньку. `branch` и `rejoin` используют custom near-target routing, чтобы поворот происходил около target node.
